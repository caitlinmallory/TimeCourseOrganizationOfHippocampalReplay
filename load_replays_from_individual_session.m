...
%posterior_difference_to_use = [{'percent_posterior_ends_masked_in_left_map'},{'percent_posterior_ends_masked_in_right_map'}];
...
inds_to_remove = [];
for ind = 1:length(decoder_events_sub.replayEvents)
    if isempty(decoder_events_sub.replayEvents(ind).duration_og)
        inds_to_remove = [inds_to_remove; ind];
    end
end
decoder_events_sub.replayEvents(inds_to_remove) = [];
t = struct2table(decoder_events_sub.replayEvents);
t.session_percent_correct_directional_assignment(:) = percent_correct_directional_assigment;
t.session_median_decoding_error(:) = median_decoding_error;
t.replay_spikeDensity_power = t.replay_spikeDensity_power;
t.replay_ripple_power=t.replay_ripple_power;
...
[~,map_with_max_posterior] = max([t.percent_posterior_in_left_map t.percent_posterior_in_right_map],[],2);
% In the case of John's filtering method, it's possible to detect a smooth
% sequence using the map with less posterior. we want to throw these events
% out.
...
t.good_candidate_event = ...
    t.best_map == map_with_max_posterior & ...
