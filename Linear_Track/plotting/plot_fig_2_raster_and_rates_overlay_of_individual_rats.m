...
% hist_bin_centers_plot = (hist_edges(1:end-1)+hist_edges(2:end))/2;
...
events_to_plot = [{'reverse_congruent_replays_spike'},{'forward_congruent_replays_spike'}];
laser_state_to_plot = {'off'};
replay_detection_type = {'spike'};
figure_titles = {'spike, laser off'};
...
%       xticklabels(num2cell(-0.5*sz:2:0))
   ...
   elseif strcmp(laser_state_to_plot{i},'both_combined')
    ...
    ylabel('Events/s')
    ylim(fig_ylim_diff)
    %     title('Forward - Reverse','Interpreter','none')
