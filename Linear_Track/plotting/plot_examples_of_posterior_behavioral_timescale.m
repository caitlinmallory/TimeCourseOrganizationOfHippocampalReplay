...
shiftSizeDecoding = 0.4;
% load binDecoding_02.mat
...
Position_Data_sub_og_sampling = Position_Data_sub;
...
figure('Position',[350 474 500 220])
% inds_plotted = gca().XLim;
% inds_plotted = round(inds_plotted); % has to be an index

inds_plotted = [3520 3920];

figure_start_time = Position_Data_sub(inds_plotted(1),1);
figure_end_time = Position_Data_sub(inds_plotted(2),1);
...
x_inds_to_label = figure_start_ind(1):15:figure_end_ind(1);

Position_Data_sub(Position_Data_sub(:,2)<0,2) = deal(0);
Position_Data_sub(Position_Data_sub(:,2)>size(posterior,2),2)= deal(size(posterior,2));

ax1 = subplot(2,1,1);
yyaxis left
L1 = plot(figure_x_inds,Position_Data_sub(figure_x_inds,2),'Color','k','LineStyle','-','LineWidth',2); hold on;
yyaxis right
L1 = plot(figure_x_inds,Position_Data_sub(figure_x_inds,5),'Color',[0.5 0.5 0.5],'LineStyle','-','LineWidth',2); hold on;
xlim([figure_x_inds(1) figure_x_inds(end)])

ax1.YAxis(1).Limits = [0 size(posterior,2)];
ax1.YAxis(2).Limits = [0 100];
yyaxis left
...
%%
% figure('Position',[350 474 1000 350])
figure('Position',[350 474 800 280])
...
yyaxis right
L2 = plot(figure_x_inds,zscored_sd_sub(figure_x_inds,3),'Color',[115 82 68]./260,'LineStyle','-');
hold on
L3 = plot(figure_x_inds,zscored_ripple_power(figure_x_inds,7),'Color',[194 150 130]./260,'LineStyle','-','Marker','none');
...
%print(['Example_run_posterior_over_stopping_period_' num2str(stopping_period)],'-dpdf','-bestfit')
saveas(gcf,fullfile(fig_path,['Example_run_posterior_over_stopping_period_' num2str(stopping_period)]),'pdf')
