...
for i = 1:size(stopping_period_boundaries,1)
    ...
    stopping_period_duration(i) = length(stopping_period_inds)*(1/posSampRate);

    if stopping_period_duration(i)==0
        continue
    end
...
% t2.(event_types{event_type})(:) = last_event_times(event_type,:);
end
...
save('replay_by_stopping_period_aligned_to_drink_onset','t2','first_event_time_table','last_event_time_table','event_hists');
